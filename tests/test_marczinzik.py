"""Tests for the Marczinzik–Böhmler dataset loader (pure Python, no Docker).
...
``tests/test_marczinzik_isolation.py``. This module checks what can be checked
cheaply on every run: the manifest census (every research-category statement
of the two vendored files -- see the dataset's ``NOTICE.md``), the
...
from apn.dataset import MARCZINZIK_DIR, load_manifest, marczinzik_dataset
...
# The universe: one member per @[category research ...] theorem in the two
# vendored files.
EXPECTED = {
...
def test_manifest_census() -> None:
    rows = load_manifest(MARCZINZIK_DIR)
    assert {r.id: r.source for r in rows} == EXPECTED
    assert all(r.excluded is None for r in rows)


def test_manifest_row_shape() -> None:
    for row in load_manifest(MARCZINZIK_DIR):
        assert (MARCZINZIK_DIR / row.source).is_file(), row.id
        # Both conjectures are open; there is nothing recorded solved.
        assert row.extra["category"] == "research open", row.id
        assert row.statement is None, row.id
        assert (MARCZINZIK_DIR / row.statement_path).is_file(), row.id


def test_spec_files_match_manifest_exactly() -> None:
    rows = load_manifest(MARCZINZIK_DIR)
    expected = sorted((MARCZINZIK_DIR / r.statement_path).name for r in rows)
    on_disk = sorted(p.name for p in (MARCZINZIK_DIR / "Isolated").glob("*.lean"))
    assert on_disk == expected


def test_dataset_loads_all_samples() -> None:
    ds = marczinzik_dataset()
    assert len(ds) == 2
...
def test_dataset_sample_shape() -> None:
    ds = marczinzik_dataset(names=["Nakayama.nakayama_conjecture"])
    assert len(ds) == 1
    sample = ds[0]
...
def test_category_never_reaches_sample_metadata() -> None:
    ...
    for sample in marczinzik_dataset():
        assert sample.metadata is not None
...
def test_dataset_names_filter_unknown_raises() -> None:
    with pytest.raises(ValueError, match="unknown or excluded"):
        marczinzik_dataset(names=["does_not_exist"])
...
def test_sketches_end_with_disproof_declaration() -> None:
    for row in load_manifest(MARCZINZIK_DIR):
        text = (MARCZINZIK_DIR / row.statement_path).read_text()
...
def test_only_target_and_disproof_theorems_survive() -> None:
    ...
    # tests/test_marczinzik_isolation.py.
    for row in load_manifest(MARCZINZIK_DIR):
        text = (MARCZINZIK_DIR / row.statement_path).read_text()
