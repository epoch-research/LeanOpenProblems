...
``apn/data/wikipedia_autoformalized/Isolated/`` specs.

``scripts/generate_wikipedia_autoformalized_isolated.py`` only *writes* the
...
* **Rewrite certificates** -- the target's *elaborated* statement must relate
  ...
  generation and cross-checks against the manifest's ``answer_form``: equal
  for the plain members, and the pinned per-form ``Iff``-wrapper inserted at
  the conclusion boundary for the rewritten ``answer(sorry) ↔ P`` forms (see
  ``scripts.fc_statements.answer_certified``). Both sides are compared after
  erasing elaboration-context display artifacts (``normalize_hygiene`` --
  α-equivalence), so Lean's own elaborator, not the regex, vouches that the
  shipped ``P`` means what the source statement meant.
...
from scripts.wikipedia_autoformalized_isolation import ISOLATED_DIR, SOURCES_DIR
...
def kept_rows() -> list[SampleRow]:
    ...
    return [r for r in load_manifest(WIKIPEDIA_AUTOFORMALIZED_DIR) if r.excluded is None]
...
async def iso_data(kept_rows: list[SampleRow]) -> IsoData:
    ...
    arrangement as ``tests/test_erdos_isolation.py::iso_data``, for the same
    reasons.
    """
    pin = fc_commit(WIKIPEDIA_AUTOFORMALIZED_DIR)
    util_module = fc_profile(pin).util_module
    async with generate_env("pytest_wikipedia_autoformalized_isolation", pin) as env:
...
async def test_isolated_files_are_structurally_correct(
    ...
    (the cut's prediction) + the derived disproof, with the target's statement
    certified against the source per its re-detected ``answer(...)`` form --
    which must also match the manifest's recorded ``answer_form``."""
    failures: list[str] = []
...
async def test_no_example_commands_survive(
    ...
    offenders = []
    for row in kept_rows:
        src = (WIKIPEDIA_AUTOFORMALIZED_DIR / row.statement_path).read_bytes()
        stem = row.statement_path.removeprefix("Isolated/").removesuffix(".lean")
...
async def test_disproof_declarations_certified(
    ...
    elaborated type is the target statement's negation."""
    failures: list[str] = []
    for row in kept_rows:
        stem = row.statement_path.removeprefix("Isolated/").removesuffix(".lean")
