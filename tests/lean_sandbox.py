...
def generate_compose_file(fc_commit: str) -> str:
    ...
    stage at the given FC pin. Reuses ``apn.task``'s build-section +
    version-tag helpers, so the image is current by construction (``build:``
    rebuilds from the Dockerfile) and never pinned to a stale fixed tag."""
    tag = get_identifier_for_image("generate", fc_commit)
    content = yaml.safe_dump(
        {
            "services": {
                "default": {
                    "image": f"{IMAGE_REPOSITORY}:{tag}",
                    "build": _build_section("generate", fc_commit),
                    "init": True,
