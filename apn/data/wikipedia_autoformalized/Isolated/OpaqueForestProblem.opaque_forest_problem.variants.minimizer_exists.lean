...
theorem opaque_forest_problem.variants.minimizer_exists :
    ∀ K : Set ℝ², IsCompact K → Convex ℝ K → (interior K).Nonempty →
      ∃ B, IsOpaqueSet K B ∧ ∀ B', IsOpaqueSet K B' → μH[1] B ≤ μH[1] B' := by
  sorry

end OpaqueForestProblem

theorem OpaqueForestProblem.opaque_forest_problem.variants.minimizer_exists.disproof : ¬ (type_of% @OpaqueForestProblem.opaque_forest_problem.variants.minimizer_exists) := sorry
