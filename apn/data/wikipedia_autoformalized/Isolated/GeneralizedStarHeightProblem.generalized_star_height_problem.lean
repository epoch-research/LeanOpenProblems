...
theorem generalized_star_height_problem :
    ∃ k : ℕ, ∀ (α : Type) [Finite α] (L : Language α), L.IsRegular →
      ∃ e : GeneralizedRegularExpression α, e.matches' = L ∧ e.starHeight ≤ k := by
  sorry

end GeneralizedStarHeightProblem

theorem GeneralizedStarHeightProblem.generalized_star_height_problem.disproof : ¬ (type_of% @GeneralizedStarHeightProblem.generalized_star_height_problem) := sorry
