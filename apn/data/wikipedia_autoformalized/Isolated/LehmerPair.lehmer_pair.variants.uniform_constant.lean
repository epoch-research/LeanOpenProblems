...
theorem lehmer_pair.variants.uniform_constant :
    ∃ C > 5 / 4, {p : ℝ × ℝ | IsLehmerPair C p.1 p.2}.Infinite := by
  sorry

end LehmerPair

theorem LehmerPair.lehmer_pair.variants.uniform_constant.disproof : ¬ (type_of% @LehmerPair.lehmer_pair.variants.uniform_constant) := sorry
