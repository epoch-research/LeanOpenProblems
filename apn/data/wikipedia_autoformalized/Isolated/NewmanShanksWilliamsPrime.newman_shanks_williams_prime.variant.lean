...
theorem newman_shanks_williams_prime.variant :
    {m : ℕ | (nswSeq (2 * m + 1)).Prime}.Infinite := by
  sorry

end NewmanShanksWilliamsPrime

theorem NewmanShanksWilliamsPrime.newman_shanks_williams_prime.variant.disproof : ¬ (type_of% @NewmanShanksWilliamsPrime.newman_shanks_williams_prime.variant) := sorry
