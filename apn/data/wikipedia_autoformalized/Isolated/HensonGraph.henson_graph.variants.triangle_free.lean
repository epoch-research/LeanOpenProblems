...
theorem henson_graph.variants.triangle_free :
    ∀ (V : Type) (G : SimpleGraph V), IsHensonGraph 3 G →
      HasFiniteModelProperty G := by
  sorry

end HensonGraph

theorem HensonGraph.henson_graph.variants.triangle_free.disproof : ¬ (type_of% @HensonGraph.henson_graph.variants.triangle_free) := sorry
