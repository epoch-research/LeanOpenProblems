...
/--
The **Zariski–Lipman conjecture**, local form: let $R$ be the local ring of a point on a complex
algebraic variety, i.e. a reduced local ring essentially of finite type over $\mathbb{C}$. If
$\operatorname{Der}_{\mathbb{C}}(R)$ is a free $R$-module, then $R$ is a regular local ring.
Since $\mathbb{C}$ is perfect, regularity of such an $R$ is equivalent to $R$ being formally
smooth over $\mathbb{C}$ (see `Algebra.IsSmoothAt`).
-/
theorem zariski_lipman_conjecture.variants.localRing (R : Type*) [CommRing R] [IsLocalRing R]
    [IsReduced R] [Algebra ℂ R] [Algebra.EssFiniteType ℂ R]
    [Module.Free R (Derivation ℂ R R)] :
    Algebra.FormallySmooth ℂ R := by
  sorry

end ZariskiLipmanConjecture

theorem ZariskiLipmanConjecture.zariski_lipman_conjecture.variants.localRing.disproof : ¬ (type_of% @ZariskiLipmanConjecture.zariski_lipman_conjecture.variants.localRing) := sorry
