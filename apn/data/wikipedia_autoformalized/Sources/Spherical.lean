...
/-- The relative interior of a simplex is contained in the simplex. -/
@[category API, AMS 51]
theorem relInterior_subset : T.relInterior ⊆ T.toSet :=
  fun _ ⟨hx, t, ht, hxt⟩ => ⟨hx, t, fun i => (ht i).le, hxt⟩
...
/-- The inward normals are dual to the vertices with respect to the ambient form. -/
@[category API, AMS 51]
theorem form_innerNormal_vertex (k j : Fin 4) :
    M.form (T.innerNormal k) (T.vertex j) = if j = k then 1 else 0 := by
  have h := LinearMap.BilinForm.apply_dualBasis_left M.form_nondegenerate T.basis k j
  rwa [basis, coe_basisOfLinearIndependentOfCardEqFinrank] at h
...
/-- The simplices of a polyhedron form a dissection of its underlying set. -/
@[category API, AMS 51]
theorem isDissection : M.IsDissection P.simplex P.toSet := ⟨P.disjoint, rfl⟩
...
/-- The standard inner product in coordinates. -/
@[category API, AMS 51]
theorem sphericalForm_apply (x y : ℝ⁴) :
    sphericalForm x y = x 0 * y 0 + x 1 * y 1 + x 2 * y 2 + x 3 * y 3 := by
  simp [sphericalForm, Matrix.toBilin'_apply', dotProduct, Fin.sum_univ_four]

/-- The Minkowski form in coordinates. -/
@[category API, AMS 51]
theorem minkowskiForm_apply (x y : ℝ⁴) :
    minkowskiForm x y = -(x 0 * y 0) + x 1 * y 1 + x 2 * y 2 + x 3 * y 3 := by
  simp [minkowskiForm, Matrix.toBilin'_apply', Matrix.mulVec_diagonal, dotProduct,
    Fin.sum_univ_four]
...
/-- The inward normals of the orthant tetrahedron are the standard basis vectors. -/
@[category test, AMS 51]
theorem orthant_innerNormal (k : Fin 4) : orthant.innerNormal k = Pi.single k 1 := by
  ext j
  have h := orthant.form_innerNormal_vertex k j
  change sphericalForm (orthant.innerNormal k) (Pi.single j 1) = _ at h
  rw [sphericalForm_single_right] at h
  rw [h, Pi.single_apply]

/-- All dihedral angles of the orthant tetrahedron are right angles. -/
@[category test, AMS 51]
theorem orthant_dihedralAngle (k l : Fin 4) (h : k ≠ l) : orthant.dihedralAngle k l = π / 2 := by
  simp [Model.Simplex.dihedralAngle, sphere, orthant_innerNormal, sphericalForm_single_single, h]

/-- All edges of the orthant tetrahedron have length `π / 2`. -/
@[category test, AMS 51]
theorem orthant_edgeLength (i j : Fin 4) (h : i ≠ j) : orthant.edgeLength i j = π / 2 := by
  simp [Model.Simplex.edgeLength, sphere, orthant, sphericalForm_single_single, h]

/-- The Dehn invariant of the orthant tetrahedron vanishes, as it must: sixteen copies of it tile
`S³`, which has no edges. -/
@[category test, AMS 51]
theorem orthant_dehnInvariant : orthant.dehnInvariant = 0 := by
  have h : (π / 2 : ℝ) ⊗ₜ[ℤ] ((π / 2 : ℝ) : AddCircle π) = 0 := by
    have e : (π / 2 : ℝ) = (2 : ℤ) • (π / 4) := by simp; ring
    have e' : (2 : ℤ) • (π / 2 : ℝ) = π := by simp; ring
    rw [e, TensorProduct.smul_tmul, ← e, ← AddCircle.coe_zsmul, e', AddCircle.coe_period,
      TensorProduct.tmul_zero]
  simp [Model.Simplex.dehnInvariant, orthant_dihedralAngle, orthant_edgeLength, h]

/-- The points $(\cosh r, \sinh r, 0, 0)$ of the geodesic through $(1, 0, 0, 0)$ in the direction
of the first spatial axis lie in `H³`. -/
@[category test, AMS 51]
theorem cosh_sinh_mem_hyperbolic (r : ℝ) : ![cosh r, sinh r, 0, 0] ∈ hyperbolic.points := by
  refine ⟨?_, by simpa using cosh_pos r⟩
  rw [minkowskiForm_apply]
  simp
  nlinarith [cosh_sq r]

/-- The point $(\cosh r, \sinh r, 0, 0)$ lies at hyperbolic distance $r$ from $(1, 0, 0, 0)$. -/
@[category test, AMS 51]
theorem hyperbolic_dist_cosh_sinh (r : ℝ) (hr : 0 ≤ r) :
    hyperbolic.dist ![1, 0, 0, 0] ![cosh r, sinh r, 0, 0] = r := by
  simp [hyperbolic, minkowskiForm_apply, arcosh_cosh hr]
...
theorem spherical.parts.i : answer(sorry) ↔
    ∀ P Q : sphere.Polyhedron, P.volume = Q.volume → P.dehnInvariant = Q.dehnInvariant →
      sphere.ScissorsCongruent P.toSet Q.toSet := by
  sorry

/--
**Hyperbolic case.** Must two compact polyhedra in hyperbolic $3$-space $H^3$ with the same
volume and the same Dehn invariant (in $\mathbb{R} \otimes_{\mathbb{Z}} (\mathbb{R}/\pi\mathbb{Z})$)
be scissors congruent? Here a polyhedron is a finite union of nondegenerate compact hyperbolic
tetrahedra (all vertices in $H^3$, no ideal vertices) with pairwise disjoint interiors, and
scissors congruence allows all isometries of $H^3$.
-/
@[category research open, AMS 51 52]
theorem spherical.parts.ii : answer(sorry) ↔
    ∀ P Q : hyperbolic.Polyhedron, P.volume = Q.volume → P.dehnInvariant = Q.dehnInvariant →
