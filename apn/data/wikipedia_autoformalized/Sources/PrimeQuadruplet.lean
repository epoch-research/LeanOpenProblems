...
/-- The first prime quadruplet is $\{5, 7, 11, 13\}$. -/
@[category test, AMS 11]
theorem prime_quadruplet_five :
    5 ∈ {p : ℕ | Prime p ∧ Prime (p + 2) ∧ Prime (p + 6) ∧ Prime (p + 8)} := by
  simp only [Set.mem_setOf_eq, ← Nat.prime_iff]
  norm_num
