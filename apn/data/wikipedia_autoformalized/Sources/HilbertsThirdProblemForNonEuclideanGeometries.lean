...
/-- The spanning vectors lie in the simplicial cone. -/
@[category API, AMS 51 52]
theorem mem_simplicialCone_self (v : Fin 4 → V) (i : Fin 4) : v i ∈ simplicialCone v := by
...
/-- The vertices of a spherical tetrahedron lie in the tetrahedron. -/
@[category test, AMS 51 52]
theorem vertex_mem_toSet (Δ : Simplex) (i : Fin 4) : Δ.vertex i ∈ Δ.toSet :=
  ⟨by simpa using Δ.norm_vertex i, mem_simplicialCone_self _ i⟩
...
/-- Two distinct vertices of the orthant are at spherical distance $\pi / 2$. -/
@[category test, AMS 51 52]
theorem angle_orthant_vertex {i j : Fin 4} (h : i ≠ j) :
    InnerProductGeometry.angle (orthant.vertex i) (orthant.vertex j) = π / 2 := by
  rw [← InnerProductGeometry.inner_eq_zero_iff_angle_eq_pi_div_two]
  simp [orthant, EuclideanSpace.inner_single_left, h]

/-- All dihedral angles of the orthant are right angles. -/
@[category test, AMS 51 52]
theorem dihedralAngle_orthant {i j k l : Fin 4} (hki : k ≠ i) (hkj : k ≠ j) (hli : l ≠ i)
    (hlj : l ≠ j) (hkl : k ≠ l) : dihedralAngle orthant.vertex i j k l = π / 2 := by
  have h : ∀ m, m ≠ i → m ≠ j →
      (Submodule.span ℝ {orthant.vertex i, orthant.vertex j})ᗮ.starProjection
        (orthant.vertex m) = orthant.vertex m := by
    intro m hmi hmj
    rw [Submodule.starProjection_eq_self_iff, Submodule.mem_orthogonal']
    intro u hu
    obtain ⟨a, b, rfl⟩ := Submodule.mem_span_pair.1 hu
    simp [orthant, EuclideanSpace.inner_single_left, hmi, hmj]
  rw [dihedralAngle, h k hki hkj, h l hli hlj,
    ← InnerProductGeometry.inner_eq_zero_iff_angle_eq_pi_div_two]
  simp [orthant, EuclideanSpace.inner_single_left, hkl]

/-- The class of $\pi / 2$ vanishes in $\mathbb{R}/\mathbb{Q}\pi$. -/
@[category API, AMS 51 52]
theorem mk_pi_div_two : (Submodule.Quotient.mk (π / 2) : ℝ ⧸ ratPi) = 0 := by
  rw [Submodule.Quotient.mk_eq_zero, ratPi, Submodule.mem_span_singleton]
  exact ⟨1 / 2, by simp [Rat.smul_def]; ring⟩

/-- The Dehn invariant of the orthant vanishes. -/
@[category test, AMS 51 52]
theorem Simplex.dehnInvariant_orthant : orthant.dehnInvariant = 0 := by
  simp [Simplex.dehnInvariant, edgeTerm, angle_orthant_vertex, mk_pi_div_two]
...
/-- The vertices of a hyperbolic tetrahedron lie in the tetrahedron. -/
@[category test, AMS 51 52]
theorem vertex_mem_toSet (Δ : Simplex) (i : Fin 4) : Δ.vertex i ∈ Δ.toSet :=
  ⟨Δ.vertex_mem i, mem_simplicialCone_self _ i⟩
...
/-- The vertices of the trirectangular tetrahedron lie in $H^3$. -/
@[category test, AMS 51 52]
theorem trirectangularVertex_mem_hyperboloid (a : ℝ) (i : Fin 4) :
    trirectangularVertex a i ∈ hyperboloid := by
  have h : Real.cosh a * Real.cosh a - Real.sinh a * Real.sinh a = 1 := by
    nlinarith [Real.cosh_sq a]
  fin_cases i <;> simp [trirectangularVertex, hyperboloid, minkowski, h, Real.cosh_pos]

/-- The dihedral angles of the trirectangular tetrahedron along the legs are right angles. -/
@[category test, AMS 51 52]
theorem dihedralAngle_trirectangularVertex {a : ℝ} (ha : a ≠ 0) :
    dihedralAngle (trirectangularVertex a) 0 1 2 3 = π / 2 := by
  have hs : Real.sinh a ≠ 0 := by simpa using ha
  have hc : Real.cosh a * Real.cosh a - Real.sinh a * Real.sinh a = 1 := by
    nlinarith [Real.cosh_sq a]
  have h1 : 1 - Real.cosh a ^ 2 = -Real.sinh a ^ 2 := by linarith [Real.cosh_sq a]
  have h2 : normalComponent (trirectangularVertex a 0) (trirectangularVertex a 1)
      (trirectangularVertex a 2) = ![0, 0, Real.sinh a, 0] := by
    ext i
    fin_cases i <;> simp [normalComponent, minkowski, trirectangularVertex, hc, h1]
    field_simp
    linear_combination Real.cosh a * h1
  have h3 : normalComponent (trirectangularVertex a 0) (trirectangularVertex a 1)
      (trirectangularVertex a 3) = ![0, 0, 0, Real.sinh a] := by
    ext i
    fin_cases i <;> simp [normalComponent, minkowski, trirectangularVertex, hc, h1]
    field_simp
    linear_combination Real.cosh a * h1
  rw [dihedralAngle, h2, h3, spacelikeAngle]
  simp [minkowski]
...
theorem hilberts_third_problem_for_non_euclidean_geometries.parts.i :
    answer(sorry) ↔ ∀ P Q : Spherical.Polyhedron,
...
/-- **Hilbert's third problem for hyperbolic geometry** (Jessen's question in $H^3$).
In hyperbolic geometry, must (compact) polyhedra with the same volume and the same Dehn invariant
be scissors-congruent? That is, is it true that any two compact polyhedra $P, Q \subseteq H^3$ with
$\operatorname{vol}(P) = \operatorname{vol}(Q)$ and $D(P) = D(Q)$ in
$\mathbb{R} \otimes_{\mathbb{Z}} \mathbb{R}/\mathbb{Q}\pi$ are scissors-congruent?
The converse (scissors-congruent polyhedra have the same volume and Dehn invariant) is known. -/
@[category research open, AMS 51 52]
theorem hilberts_third_problem_for_non_euclidean_geometries.parts.ii :
    answer(sorry) ↔ ∀ P Q : Hyperbolic.Polyhedron,
      Hyperbolic.volume P.toSet = Hyperbolic.volume Q.toSet →
      P.dehnInvariant = Q.dehnInvariant →
      Hyperbolic.ScissorsCongruent P.toSet Q.toSet := by
  sorry
