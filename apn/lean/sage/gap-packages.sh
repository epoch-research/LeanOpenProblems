...
# plus grape/guava and hap's dependency nq), taken from GAP's own packages
# tarball for the installed GAP release -- the set GAP ships as mutually
# compatible -- into the env's GAP root; the compiled three are built the way
# GAP's BuildPackages does. VERSION must be the gap-defaults release in
# sage.yaml; the GRAPE smoke in tests/test_agent_image.py catches a mismatch.
# packagemanager is in conda GAP's default PackagesToLoad but not shipped,
# hence the '#I packagemanager package is not available' on every startup
# without it.
set -eu
