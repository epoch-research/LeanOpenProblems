...
* The `sage` computer algebra system (SageMath 10) with its standard engines (PARI, GAP, Singular, Maxima, Giac, FLINT, Macaulay2, ...), `gp`, `gap`, `Singular`, `maxima`, and `M2` also on PATH, and these optional backends and databases installed: Normaliz (PyNormaliz), Giac Groebner bases and integration (sagemath-giac), CryptoMiniSat and picosat SAT backends, symengine, the GAP packages GRAPE, GUAVA, HAP, Design, QPA and QuaGroup, and the Cremona elliptic curve, Jones number field, KnotInfo, matroid, cubic Hecke and Odlyzko zeta-zero databases. `sage.features` reports which optional backends are available.
