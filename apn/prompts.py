...
* Two Lean libraries as source only, not importable: Tau Ceti at `/opt/tauceti` and Lean Pool at `/opt/lean-pool` (snapshots from {LEAN_LIBS_SNAPSHOT}). They contain formalizations beyond Mathlib. To use a result, copy the declarations you need and their dependencies into files under `{SUBMISSION_DIR}/` and adapt them to this project's Mathlib. Only copy what you need.
