...
* CAPD, a C++ library for rigorous numerics (interval arithmetic, validated ODE integration, rigorous Poincaré maps and derivatives of flows, interval Newton): compile with `g++ prog.cpp $(capd-config --cflags --libs)`; docs and a project starter are in `/usr/local/share/doc/capd`.
