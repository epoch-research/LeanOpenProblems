...
* A Lean 4 toolchain with Mathlib, plus `loogle` for searching Mathlib by constant name or type pattern (e.g. `loogle 'Nat.Prime, _ ^ _'` or `loogle '|- tsum _ = _ * tsum _'`). To inspect a goal state, insert `trace_state` into a tactic proof and run `lake env lean <file>`: the goal is printed on stdout (an unfinished proof likewise prints its unsolved goals).
* The `sage` computer algebra system (version 10), with `gp` (PARI), `gap`, `Singular`, and `maxima` also on PATH.
* `python3` with numpy, scipy, sympy, mpmath, pandas, networkx, igraph, python-flint (fast arbitrary-precision number theory), highspy (LP/MIP), python bindings for the solvers below (z3, cvc5, OR-Tools CP-SAT, pysat), snappy (SnapPy, 3-manifolds), and ore_algebra (D-finite sequences, used from sage).
